  ...
  try {
    ...
    if (!amount || !payment_method || !subscription_type) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        error: "Missing required fields: amount, payment_method, subscription_type",
      });
    }
    ...
    if (subscription_type === SubscriptionTypeEnum.MEMBER && !member_subscription_id) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        error: "member_subscription_id is required for MEMBER subscription type",
      });
    }
    ...
    const payment = await prisma.payment.create({
      data: {
        owner_subscription_id: subscription_type === SubscriptionTypeEnum.OWNER ? owner_subscription_id : null,
        member_subscription_id: subscription_type === SubscriptionTypeEnum.MEMBER ? member_subscription_id : null,
